...
from keras.layers import Dense, Input, Embedding, Lambda, Dropout, Activation, SpatialDropout1D, GlobalAveragePooling1D, merge, Flatten, Bidirectional, CuDNNGRU, Conv1D, GlobalMaxPooling1D
...
def get_av_cnn(nb_words, embedding_dim, embedding_matrix, max_sequence_length, out_size, 
               custom_weights=None, loss_fn=None):
    ...
    if (loss_fn is not None) and (custom_weights is not None):
...
def get_kmax_text_cnn(nb_words, embedding_dim, embedding_matrix, max_sequence_length, out_size):
    ...
    model.compile(loss='binary_crossentropy', optimizer=adam_optimizer, metrics=['accuracy',f1])
    model.summary()
    return model


def get_rcnn(nb_words, embedding_dim, embedding_matrix, max_sequence_length, out_size, loss_fn=None, class_weights=None):
    recurrent_units = 64
    filter1_nums = 128
    ...
    #model.compile(loss='binary_crossentropy', optimizer=adam_optimizer, metrics=['accuracy'])
    if (loss_fn is not None) and (class_weights is not None):
        model.compile(loss=loss_fn(class_weights), optimizer=adam_optimizer, metrics=['accuracy'])
    else:
        model.compile(loss='binary_crossentropy', optimizer=adam_optimizer, metrics=['accuracy'])
...
def get_av_rnn(nb_words, embedding_dim, embedding_matrix, max_sequence_length, out_size, loss_fn=None, class_weights=None):
    recurrent_units = 60
    input_layer = Input(shape=(max_sequence_length,))
    embedding_layer = Embedding(nb_words,
                                embedding_dim,
...
def get_dropout_bi_gru(nb_words, embedding_dim, embedding_matrix, max_sequence_length, out_size, loss_fn=None, class_weights=None,
    ...
    if (loss_fn is not None) and (class_weights is not None):
        model.compile(loss=loss_fn(class_weights), optimizer='adam', metrics=['accuracy',f1])
    else:
